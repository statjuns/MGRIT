...
def log_epoch(config, writer, epoch, train_res, split, scores, lang, which='ft_xe'):
    ...
    if epoch == 0 and not os.path.exists('result.csv'):
        with open(f'{lang}_result.csv', 'w') as f:
            f.write(head + '\n')

    with open(f'{lang}_result.csv', 'a') as f:
...
def evaluate_metrics(
    ...
    model.eval()
    gen, gts = defaultdict(dict), defaultdict(dict)
    ...
    with tqdm(desc=f'Epoch {epoch} - evaluation on {split}', unit='it', total=len(dataloader)) as pbar:
        ...
        for it, batch in enumerate(iter(dataloader)):
            ...
            caps_gen = [[cap] for cap in caps_gen]

            for lang in ['all_langs'] + text_field.language_list:
                # print("text_field.vocab.itos[batch['lang'][0]]: ",text_field.vocab.itos[batch['langs'][0]])
                lang_indices = [i for i, l in enumerate(batch['langs']) if text_field.vocab.itos[l] == lang] if lang != 'all_langs' else [i for i, l in enumerate(batch['langs'])]

                if lang_indices == []:
                    continue
                gt_captions = pd.Series(batch['captions']).iloc[lang_indices].tolist()
                gen_captions = pd.Series(caps_gen).iloc[lang_indices].tolist()
                for i, (gts_i, gen_i) in enumerate(zip(gt_captions, gen_captions)):
                    # if ' ' not in gts_i[0]:
                    if gts_i[0].count(' ') <= 1:
                        gen[f'{lang}'][f'{it}_{i}'] = [' '.join(list(gen.strip())) for gen in gen_i]
                        gts[f'{lang}'][f'{it}_{i}'] = [' '.join(list(gts.strip())) for gts in gts_i]
                    else:
                        gen[f'{lang}'][f'{it}_{i}'] = [gen.strip() for gen in gen_i]
                        gts[f'{lang}'][f'{it}_{i}'] = [gts.strip() for gts in gts_i]
                    if lang != 'all_langs':
                        res = {'image_id': batch['image_id'][i], 'caption': gen_i, 'lang': lang}
                        results.append(res)
    ...
    for lang in text_field.language_list + ['all_langs']:
        # print(lang)
        if len(gts[lang]) == 0:
            # print(lang)
            continue
        # print(lang)
        if list(gts[lang].keys()) != []:
            print('gts:  ', gts[lang][list(gts[lang].keys())[0]])
            print("gen:  ", gen[lang][list(gts[lang].keys())[0]])

        gt = metrics.PTBTokenizer.tokenize(gts[lang])
        ge = metrics.PTBTokenizer.tokenize(gen[lang])
        scores, _ = metrics.compute_scores(gt, ge)
        print(f'Epoch {epoch}: {split} scores: {lang} :' + str(scores) + '\n')

        if log_and_save:
            with open(f'{lang}_result.txt', 'a') as f:
                f.write(f'Epoch {epoch}: {split} scores: ' + str(scores) + '\n')
            log_epoch(config, writer, epoch, train_res, split=split, scores=scores, lang=lang, which=which)
...
def train_sc(model,
    ...
    with tqdm(desc='Epoch %d - train' % epoch, unit='it', total=len(dataloaders['train_dict'])) as pbar:
        for it, batch in enumerate(dataloaders['train_dict']):
            ...
            optimizers['model'].zero_grad()
            optimizers['backbone'].zero_grad()
            bos_idx = text_field.vocab.stoi[batch['langs'][0]]
            # print(batch['langs'])
            bos_idx = batch['langs'][0]
            # print('bos_idx!!!!!:  ', bos_idx) # TODO delete print
