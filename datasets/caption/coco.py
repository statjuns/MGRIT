...
class DictionaryCollator:
    ...
    def __call__(self, batch):
        ...
        if len(batch[0])>3:
            langs = [item[3] for item in batch]
            outputs['langs'] = langs
        return outputs
...
class PairedCollator(DictionaryCollator):
    ...
    def __call__(self, batch):
        ...
        if 'langs' in b:
            ...
            padded = []
            for l, c in zip(langs, captions):
                # caption = [self.bos_idx] + c + [self.eos_idx] + [self.pad_idx] * (max_len - len(c))
                caption = [l] + c + [self.eos_idx] + [self.pad_idx] * (max_len - len(c))
                # print(caption)
...
class CPairedDataset:
    ...
    def __getitem__(self, idx):
        ...
        if example.lang is not None:
            lang = example.lang
            return img, caption, image_id, lang
        else:
...
class TestDataset:
    ...
    def __getitem__(self, idx):
        ...
        if 'lang' in item:
            lang = item['lang'] # TODO annotation lang 내용추가
            return img, img_id, lang
        else:
            return img, img_id
...
class CDictionaryDataset:
    ...
    def __init__(self, examples, image_field, overfit=False):
        ...
        for example in examples:
            ...
            if example.lang is not None:
                self.img2lang[example.image] = example.lang
    ...
    def __getitem__(self, idx):
        ...
        lang = self.img2lang[img_path]
        # print(img_path)
        img = self.image_field.preprocess(img_path)
        return img, captions, image_id, lang # TODO lang
    ...
    def get_samples(self, roots, ids_dataset=None):
        ...
        for split in splits:
            ...
            for index in tqdm(range(len(ids))):
                ...
                """
                {'image_id': 435750, 
                'image': '/home/naver1/Documents/sjjung/grit/data/coco/train2014/COCO_train2014_000000435750.jpg', 
                'text': 'A traffic signal and street sign in a city.', 
                'tokens': [4, 180, 963, 10, 24, 59, 8, 4, 84]}
                """


                #TODO lang : lang_code 추가
...
class CUSTOM_DATA(CPairedDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        roots['train'] = {
            ...
            'cap': os.path.join(ann_root, 'train_text')
        }
        roots['valid'] = {
            'img': os.path.join(img_root, 'valid'),
            'cap': os.path.join(ann_root, 'valid_text')
        }
        roots['test'] = {
            'img': os.path.join(img_root, 'test'),
            'cap': os.path.join(ann_root, 'test_text')
        }
        roots['trainrestval'] = {
            'img': (roots['train']['img'], roots['valid']['img']),
            'cap': (roots['train']['cap'], roots['valid']['cap'])
    ...
    def get_samples(self, roots, ids_dataset=None):
        ...
        for split in splits:
            ...
            cap_root = roots[split]['cap']

            caption_fns = os.listdir(roots[split]['cap'])

            for index in tqdm(range(len(caption_fns))):

                caption_fn = caption_fns[index]
                with open(os.path.join(cap_root, caption_fn), 'r') as f:
                    caption = f.readlines()
                    caption, lang = caption[0].strip(), caption[1].strip()
                img_id = int(caption_fn.split("_")[-1].split('.')[0])
                filename = caption_fn.split(".")[0] + '.jpg'
                # print(lang, self.text_field.vocab.stoi[lang])
                ...
                example = Example.fromdict({
                    ...
                    'text': caption,
                    'tokens': [self.text_field.vocab.stoi[w] for w in self.text_field.preprocess(caption)],
                    'lang' : self.text_field.vocab.stoi[lang] #TODO
                    # 'lang': self.text_field.vocab.stoi['<bos>']  # TODO
...
def build_coco_dataloaders(config=None, mode='freezing', device='cpu', debug=False):
    overfit = config.dataset.overfit
    transform = get_transform(config.dataset.transform_cfg)
    if os.path.isfile(config.dataset.vocab_path):
        text_field = TextField(vocab_path=config.dataset.vocab_path, config=config)
    else:
        print('build vocab!!!')
        text_field = TextField(vocab_path=config.dataset.vocab_path, build_vocab=True, config=config)
    ...
    #TODO COCO class, custom dataset에서는 어떻게 사용해야하는지 방법찾기
    if config.dataset.data_name == "COCO":
        examples = COCO(None, text_field, **config.dataset).split_examples()
    elif config.dataset.data_name == "CUSTOM":
        examples = CUSTOM_DATA(None, text_field, **config.dataset).split_examples()
    ...
    if not debug:
        ...
        datasets = {
            'train': CPairedDataset(examples['train'][:1000], train_field, overfit=overfit),
            'valid': CPairedDataset(examples['valid'][:1000], valid_field, overfit=overfit),
            'train_dict': CDictionaryDataset(examples['train'][:1000], train_field, overfit=overfit),
            'valid_dict': CDictionaryDataset(examples['valid'][:1000], valid_field, overfit=overfit),
            'test_dict': CDictionaryDataset(examples['test'][:1000], valid_field, overfit=overfit),
        }
