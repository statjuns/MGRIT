...
# nllb_tokenizer = spm.SentencePieceProcessor()
# vocab_file = "/home/naver1/Documents/sjjung/grit/data/coco/sentencepiece.bpe.model"
# nllb_tokenizer.load(vocab_file)
# nllb_tokenizer_func = nllb_tokenizer.encode_as_pieces

# def tokenize(s):
#     return [tok for tok in nllb_tokenizer_func(s) if tok != '▁']

# def tokenize_cn(s):
#     return [tok for tok in jie_ba_tokenizer(s)]
...
class TextField:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.config = config
        # self.nllb_tokenizer = spm.SentencePieceProcessor()
        # vocab_file = "/home/naver1/Documents/sjjung/grit/data/coco/sentencepiece.bpe.model"
        # self.nllb_tokenizer.load(vocab_file)
        self.nllb_tokenizer = AutoTokenizer.from_pretrained("facebook/nllb-200-distilled-600M",)
        self.language_list = self.nllb_tokenizer.special_tokens_map['additional_special_tokens']
    ...
    def preprocess(self, caption):
        ...
        # caption = tokenize_cn(caption.rstrip('\n'))
        if self.remove_punctuation:
            caption = [w for w in caption if w not in self.punctuations]
        return caption

    def tokenize(self, caption):
        caption = [tok for tok in self.nllb_tokenizer.tokenize(caption.rstrip('\n'))[1:] if tok!='▁']
    ...
    def build_vocab(self, *sources, **kwargs):
        ...
        """
        #TODO build multilingual vocab(COCO + COCO-CN)
        counter = Counter()

        with open(self.config.dataset.caption_source_path, 'rb') as f:
        ...
        specials = list(
            OrderedDict.fromkeys(
                [tok for tok in [self.unk_token, self.pad_token, self.init_token, self.eos_token] + self.nllb_tokenizer.special_tokens_map['additional_special_tokens'] if tok is not None]))
    ...
    def pad(self, minibatch):
        ...
        padded, lengths = [], []
        for x in minibatch:
            #TODO language specific init token
            if self.pad_first:
    ...
    def decode(self, word_idxs, join_words=True):
        ...
        for wis in word_idxs:
            ...
            if join_words:
                ...
                #ss
                # print(caption)
                caption = self.nllb_tokenizer.convert_tokens_to_string(caption)
            captions.append(caption)

        # self.nllb_tokenizer.batch_decode(word_idxs)
        return captions