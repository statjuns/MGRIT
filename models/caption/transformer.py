...
class Transformer(BaseCaptioner):
    ...
    def forward(self,
        ...
        # print('seq:  ', seq)
        # self.bos_idx = bos_idx
        # print('use_beam_search:  ', use_beam_search)
    ...
    def step(self, timestep, prev_output,  samples,device, bos_idxs, mode='teacher_forcing', **kwargs):
        it = None
        if mode == 'teacher_forcing':
            ...
            if timestep == 0:
                ...
                it = torch.tensor(bos_idxs).to(device) # TODO language specific token
                it = it.reshape(it.shape[0], 1).long()
                # print('bos_idx!: ', self.bos_idx)
                # print(it) #TODO del print
            else:
                it = prev_output
        # print('it: ',it) # TODO del print
    ...
    def iter(self, timestep, samples, outputs, return_probs, batch_size, bos_idxs, device, beam_size=5, eos_idx=3, **kwargs):
        cur_beam_size = 1 if timestep == 0 else beam_size
        # print(bos_idxs) #TODO delete print
        word_logprob = self.step(timestep, self.selected_words, samples,device=device, bos_idxs = bos_idxs,mode='feedback', **kwargs)
