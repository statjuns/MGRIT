...
def main(gpu, config):
    ...
    if start_epoch < config.optimizer.freezing_xe_epochs:
        ...
        if getattr(config.optimizer, 'freeze_detector', False):
            for p, n in model.named_parameters():
                if 'detector' in n:
                    p.requires_grad = False
        elif not os.path.isfile(config.dataset.hdf5_path):
    ...
    if start_epoch < config.optimizer.freezing_xe_epochs \
        ...
        dataloaders, samplers = build_coco_dataloaders(config, mode='freezing', device=device, debug=config.exp.debug)
    else:
        model.module.cached_features = False
        dataloaders, samplers = build_coco_dataloaders(config, mode='finetune', device=device, debug=config.exp.debug)
    ...
    print("len(dataloaders['train_dict']):  ", len(dataloaders['train_dict']))
...
@hydra.main(config_path="configs/caption", config_name="mic_icbsgen_config")
def run_main(config: DictConfig) -> None:
    mp.spawn(main, nprocs=config.exp.ngpus_per_node, args=(config,))


if __name__ == "__main__":
    os.environ["DATA_ROOT"] = "/data/MIC_DATA/"
    os.environ["MASTER_ADDR"] = "localhost"
    os.environ["MASTER_PORT"] = "6699"
    run_main()