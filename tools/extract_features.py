...
class ExtractDataset(Dataset):
    ...
    def __init__(self, root, transform=None):
        ...
        #TODO train valid -> train2014, val2014
        ...
        self.img_paths = glob(os.path.join(self.root, "train/*"))
        self.img_paths += glob(os.path.join(self.root, "valid/*"))  # Karpathy val/test in val2014 dir
        self.img_paths += glob(os.path.join(self.root, "test/*"))
...
def extract_vis_features(model, config, device, rank):
    BATCH_SIZE = config.model.detector.batchsize
    model = model.eval()
